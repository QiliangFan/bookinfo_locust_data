...
def get_args():
    ...
    argparser.add_argument("--metric", type=str, required=False, help="the metric to be collected.", default=None)
    argparser.add_argument("--code", type=int, required=False, default=None)
    argparser.add_argument("--ts", type=int, required=False, default=int(time()))
            ...
            if isinstance(svc, str):
                ...
                for svc_name, versions in svc.items():
                    for version in versions:
                        istio_data = IstioData(svc_name=svc_name, version=version, **CONFIG)
                        istio_data.run()
...
if __name__ == "__main__":
    ...
    NODE_METRICS_CUM = open(os.path.join(PROJECT_PATH, "node_metrics_cumulative.txt"), "r")
